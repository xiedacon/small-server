...
async function resolveFile (root, _path) {
  _path = decodeURIComponent(parse(_path).pathname.slice(1))

  let path, mtime, size, code
  if (_path.startsWith('../') || _path.startsWith('/')) {
    [code, path, mtime] = [404, page._404, -1]
  } else {
    [code, path, mtime, size] = await _resolveFile(root, _path).catch(() => {
      return _path === ''
        ? [200, page.index, -1]
        : [404, page._404, -1]
    })
...
async function _resolveFile (root, _path) {
  ...
  if (!stats) return
    ...
    if (_path === page.suffix) return [404, page._404, -1]
    return _resolveFile(path, page.suffix)
  }
